...
活动同步只请求 Temu 当前活动报名记录，固定传入 `sessionStatus=2`；不请求活动列表、活动详情或商品上下架状态。返回后还会移除非当前嵌套场次及其站点价格，内存快照只保留当前活动价格和库存。Temu 的 `activityStock` 与 `remainingActivityStock` 位于报名记录层级；同一报名中的多个 SKU 共享这组库存，不能把单次库存差值直接视为某个 SKU 的销量。当前接口展示的是“报名总库存减当前剩余库存”的累计差值，尚未持久化相邻快照变化。
