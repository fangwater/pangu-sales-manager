...
报名库存比较和 SKC 推断状态只保存在进程内存中。SKC 首次出现时，如果只有一个活动的 `activityStock - remainingActivityStock` 大于零，就确认该活动；否则标记预警。后续分钟只有一个活动库存下降时确认或切换到该活动，没有下降时延续上一状态，多个活动同时下降时标记预警。对外状态只有 `confirmed` 和 `warning`。
