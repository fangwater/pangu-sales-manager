...
func NewClient(baseURL, appKey, appSecret, accessToken string, timeout time.Duration) *Client {
	return &Client{
		...
		httpClient: &http.Client{Timeout: timeout}, clock: time.Now,
	}
}
...
func (c *Client) Call(ctx context.Context, apiType string, parameters map[string]any, result any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, &APIError{APIType: apiType, Status: response.StatusCode, Message: err.Error(), Temporary: true, Cause: err}
	}
	var envelope struct {
	...
	if err := json.Unmarshal(raw, &envelope); err != nil {
		return raw, &APIError{APIType: apiType, Status: response.StatusCode, Message: "invalid JSON response", Temporary: response.StatusCode >= 500 || response.StatusCode == http.StatusTooManyRequests, Cause: err}
	}
